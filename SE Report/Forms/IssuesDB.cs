...
namespace SE_Report.Forms
{
    public partial class IssuesDB : Form
    {
        ...
        {
            XmlReader xmlFile;
            FileStream fs = new FileStream(@"\\lgmcfs-sp\SW\CM\SE\TOOLS\NAO MEXER\IssuesDB.xml", FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
        ...
        private void PDF()
        {
            ...
            swv.Font.Size = 12;
            swv.Alignment = Element.ALIGN_CENTER;
            PdfPCell model = new PdfPCell(new Phrase("Model: " + ModelBox.Text));
            PdfPCell buyer = new PdfPCell(new Phrase("Buyer: " + BuyerBox.Text + "    PKG" + PackageBox.Text));
            ...
            PdfPCell testCase = new PdfPCell(new Phrase(Table.Columns[2].HeaderText.ToUpper()));
            testCase.HorizontalAlignment = Element.ALIGN_CENTER;
            sheet.AddCell(testCase); // Column headers
            PdfPCell result = new PdfPCell(new Phrase(Table.Columns[0].HeaderText.ToUpper()));
            result.HorizontalAlignment = Element.ALIGN_CENTER;
            sheet.AddCell(result);
            PdfPCell comments = new PdfPCell(new Phrase(Table.Columns[1].HeaderText.ToUpper()));
            ...
            for (int j = 0; j < Table.Rows.Count; j++)
            {
                ...
                if (Table[0, j].Value != null)
                {
                    if ((string)Table[0, j].Value == "PASSED")
                    {
                        ...
                        sheet.AddCell(Table[0, j].Value.ToString());
                    }
            ...
            try
            {
                ...
                string path = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + "\\" + "ISSUES DB REPORT - " + VersionBox.Text + ".pdf";
